...
@dataclass
class UnitToUnitAugDatasetItem(object):
    index: int
    source: torch.Tensor
...
class UnitToUnitAugDataset(UnitToTextDataset):
    def __init__(
        self,
        split: str,
        is_train_split: bool,
    ...
    def pack_src_units(self, input: str) -> str:
        return input


    def unit_seq_augmentation(self, input: str, p: float, M: int) -> str:
        """Replace M consecutive units of the sequence with a random number with probability p."""
        sequence = input.split()
        n = len(sequence)
        new_sequence = sequence.copy() # Make a copy of the original sequence
      
        # be careful with unit replacement. Don't swap to special tokens
        for i in range(n):
            if random.random() < p: # With probability p, replace M consecutive units
                j = i
                while j < min(i+M, n):
                    new_sequence[j] = str(random.randint(4, self.src_dict.__len__()-1)) # Replace with a random integer
                    j += 1

        output = " ".join(new_sequence) # Convert the list of characters back to a string

        return output

    def __getitem__(self, index: int) -> UnitToUnitAugDatasetItem:

        if not self.source_is_code: 
            source = self._get_source_audio(index)
        else: 
            orig_src_unit = self.src_audio_paths[index]
            if self.src_is_aug: 
                # augmentation 
        ...
        return UnitToUnitAugDatasetItem(
            index=index,
            source=source,
            target=target,
    ...
    def _collate_target(self, samples: List[UnitToUnitAugDatasetItem]) -> torch.Tensor:
    ...
    def _collate_source(self, samples: List[UnitToUnitAugDatasetItem]) -> torch.Tensor:
    ...
    def collater(
        self, samples: List[UnitToUnitAugDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
...
class UnitToUnitAugMultitaskDataset(UnitToUnitAugDataset):
    ...
    def __getitem__(
        self, index: int
    ) -> Tuple[UnitToUnitAugDatasetItem, Dict[str, torch.Tensor]]:
    ...
    def collater(
        self, samples: List[Tuple[UnitToUnitAugDatasetItem, Dict[str, torch.Tensor]]]
    ) -> Dict:
        if len(samples) == 0:
            return {}
...
class UnitToUnitAugDatasetCreator(object):
    # mandatory columns
    ...
    def _from_list(
        ...
        dataset_cls = (
            UnitToUnitAugMultitaskDataset if has_multitask else UnitToUnitAugDataset
        )
