...
@dataclass
class UnitToTextDatasetItem(object):
    index: int
    source: torch.Tensor
...
class UnitToTextDataset(FairseqDataset):
    ...
    def get_src_lens_and_check_oov(self):
        ...
        logger.info(f"'{self.split}' has {n_oov_tokens / n_tokens * 100:.2f}% OOV in the source")
        return src_lens
    ...
    def get_tgt_lens_and_check_oov(self):
        ...
        logger.info(f"'{self.split}' has {n_oov_tokens / n_tokens * 100:.2f}% OOV in the target")
        return tgt_lens
    ...
    def __getitem__(self, index: int) -> UnitToTextDatasetItem:
    ...
    def collater(
        self, samples: List[UnitToTextDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
...
class TextTargetMultitaskData(object):
    ...
    def __init__(self, args, split, tgt_dict):
        samples = UnitToTextDatasetCreator._load_samples_from_tsv(args.data, split)
...
class SpeechToTextMultitaskDataset(UnitToTextDataset):
    ...
    def __getitem__(
        self, index: int
    ) -> Tuple[UnitToTextDatasetItem, Dict[str, torch.Tensor]]:
    ...
    def collater(
        self, samples: List[Tuple[UnitToTextDatasetItem, Dict[str, torch.Tensor]]]
    ) -> Dict:
        if len(samples) == 0:
            return {}
...
class UnitToTextDatasetCreator(object):
    # mandatory columns
    ...
    def _from_list(
        ...
        dataset_cls = (
            SpeechToTextMultitaskDataset if has_multitask else UnitToTextDataset
        )
    ...
    @classmethod
    def get_size_ratios(
        cls, datasets: List[UnitToTextDataset], alpha: float = 1.0
    ) -> List[float]:
