...
class SequenceGeneratorCopyMech(SequenceGenerator):
    def __init__(
        self,
        models,
        tgt_dict,
        ...
        if isinstance(models, EnsembleModel):
            ...
            self.model = EnsembleModelCopyMech(models)
    ...
    def generate_decoder(
        ...
        new_src_tokens = encoder_outs[0]['src_tokens'][0].to(src_tokens)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            with torch.autograd.profiler.record_function(
                "EnsembleModel: forward_copy_decoder"
            ):
                lprobs, avg_attn_scores = self.model.forward_copy_decoder(
                    new_src_tokens, 
                    tokens[:, : step + 1],
                    encoder_outs,
                    incremental_states,
...
class EnsembleModelCopyMech(EnsembleModel):
    @torch.jit.export
    def forward_copy_decoder(
        self,
        src_tokens, 
        tokens,
