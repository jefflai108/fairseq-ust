...
class RawMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        ##############################################################################
        #
        # don't use official multi_head_attn so that we can return the raw attn weights
        #
        ###############################################################################
        #if (
        #    not self.onnx_trace
        ...
        #    else:
        #        print('called pytorch F.multi_head_attention_forward')
        ...
        if saved_state is not None:
            ...
            key_padding_mask = RawMultiheadAttention._append_prev_key_padding_mask(
        ...
        if not self.use_fused_softmax or torch.jit.is_scripting():
            raw_attn_weights = attn_weights # take the pre-softmax attention 
        ...
        if need_weights:
            ...
            raw_attn_weights = raw_attn_weights.view(
                bsz, self.num_heads, tgt_len, src_len
            ).transpose(1, 0)
            if not need_head_weights:
                # average attention weights over heads
                attn_weights = attn_weights.mean(dim=0)
                raw_attn_weights = raw_attn_weights.mean(dim=0)
