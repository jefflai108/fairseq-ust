    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def _load_from_state_dict(
        ...
        if self.BT_version:
            ...
            state_dict[new_name + "out_proj_bias"] = state_dict[
                old_name + "out_proj.bias"
            ]
...
class RawTransformerDecoderLayerBase(nn.Module):
    """Decoder layer block.
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.alpha2 = None
        # TODO : Removing alternate_decoder_ffn_embed_dim condition here, check again
        build_moe = self.is_moe_layer
        if not build_moe or cfg.moe_cmr:
            self.activation_fn = utils.get_activation_fn(
                activation=str(cfg.activation_fn)
                if cfg.activation_fn is not None
    ...
    def build_encoder_attention(self, embed_dim, cfg):
        # for encoder_attention, we use a custom implementation where 
        # raw attn_weights are also returned for later use. 
        from fairseq.modules import RawMultiheadAttention
        return RawMultiheadAttention(
            embed_dim,
            cfg.decoder.attention_heads,
    ...
    def forward(
        ...
        if self.onnx_trace and incremental_state is not None:
            ...
            return x, attn, self_attn_state, raw_attn
        return x, attn, None, l_aux, raw_attn
...
# backward compatible with the legacy argparse format
class RawTransformerDecoderLayer(RawTransformerDecoderLayerBase):
    def __init__(
        self,
        args,
        no_encoder_attn=False,
