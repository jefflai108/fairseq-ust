...
def module_name_fordropout(module_name: str) -> str:
    if module_name == "RawTransformerDecoderBase":
        return "RawTransformerDecoder"
    else:
        return module_name


class RawTransformerDecoderBase(FairseqIncrementalDecoder):
    """
    Transformer decoder consisting of *cfg.decoder.layers* layers. Each layer
    is a :class:`RawTransformerDecoderLayer`.
    ...
    def build_decoder_layer(self, cfg, no_encoder_attn=False, is_moe_layer=False):
        layer = RawTransformerDecoderLayer(cfg, no_encoder_attn, is_moe_layer=is_moe_layer)
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _, l_aux_i, raw_layer_attn = layer(
                x,
                enc,
                padding_mask,
                incremental_state,
            ...
            if layer_attn is not None and idx == alignment_layer:
                ...
                raw_attn = raw_layer_attn.float().to(x)
        #print(x.shape) # (seq_T, B, hidden_dim)
        #print(attn.shape) # (num_heads, B, seq_T, seq_S)
        #print(raw_attn.shape)
        #print(raw_attn[:, 0, :, :])
        #print(attn[:, 0, :, :])
        #print('######3break#######')
        #print(torch.sum(raw_attn[:, 0, :, :], dim=-1))
        #print(torch.sum(attn[:, 0, :, :], dim=-1))
        #print('######3break#######')
        #print(raw_attn[:, -1, :, :].mean(dim=0))
        #print(attn[:, -1, :, :].mean(dim=0))

        if attn is not None:
            if alignment_heads is not None:
                attn = attn[:alignment_heads]
                raw_attn = raw_attn[:alignment_heads]

            # average probabilities over heads, only do this for `attn`
            # we don't perform averaging for `raw_attn`
            attn = attn.mean(dim=0) # (B, seq_T, Seq_S)

        results["attn"] = [attn]
        results["raw_attn"] = [raw_attn]
...
class RawTransformerDecoder(RawTransformerDecoderBase):
    def __init__(
        self,
        args,
        dictionary,
