...
#@register_model("u2t_transformer")
#class U2TTransformerModel(FairseqEncoderDecoderModel):
...
#        encoder = U2TTransformerEncoder(args)
...
class U2TTransformerEncoder(FairseqEncoder):
    """Unit-to-text Transformer encoder that consists of input subsampler and
    Transformer encoder."""
    ...
    def _forward(self, src_tokens, src_lengths, return_all_hiddens=False):
        print(src_tokens)
        print(src_lengths)
        #x, input_lengths = self.subsample(src_tokens, src_lengths) # get rid of subsampling 
        x, input_lengths = src_tokens, src_lengths
        print(x)
        print(input_lengths)
        
        x = self.embed_scale * x # keep, reference TransformerEncoderBase

        encoder_padding_mask = lengths_to_padding_mask(input_lengths)
        positions = self.embed_positions(encoder_padding_mask).transpose(0, 1)
        x += positions
        x = self.dropout_module(x) # keep, reference TransformerEncoderBase
        ...
        print(encoder_padding_mask.shape) # torch.Size([216, 46]) (B, T)
        print(x.shape) # torch.Size([46, 216, 256]) (T, B, C)
        exit()
        return {
