...
if [ $L -eq 50 ]; then
    ######## L <= 50 #######
    TRAIN_SET="train_mined_t1.07_filter50"
    VALID_SET="valid_vp_filter50"
fi

if [ $L -eq 100 ]; then
    ######## L <= 100 #######
    TRAIN_SET="train_mined_t1.07_filter100"
    VALID_SET="valid_vp_filter100"
fi

if [ $L -eq 200 ]; then
    ######## L <= 200 #######
    TRAIN_SET="train_mined_t1.07_filter200"
    VALID_SET="valid_vp_filter200"
fi

if [ $L -eq 250 ]; then
    ######## L <= 250 #######
    TRAIN_SET="train_mined_t1.07_filter250"
    VALID_SET="valid_vp_filter250"
fi

if [ $L -eq 400 ]; then
    ######## L <= 400 #######
    TRAIN_SET="train_mined_t1.07_filter400"
    VALID_SET="valid_vp_filter400"
fi

if [ $L -eq 500 ]; then
    ######## L <= 500 #######
    TRAIN_SET="train_mined_t1.07_filter500"
    VALID_SET="valid_vp_filter500"
fi

if [ $L -eq 1024 ]; then
    ######## L <= 1k #######
    TRAIN_SET="train_mined_t1.07_filter1024"
...
# experimenting for `train_mined_t1.07_filter{200,250,400,500}` now. Guess 50k steps suffice.
