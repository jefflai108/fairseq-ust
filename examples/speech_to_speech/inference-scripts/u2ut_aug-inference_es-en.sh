...
TRAINED_S2S_MODEL=/data/sls/scratch/clai24/lexicon/exp/bilingual_textless_s2st/${SRC}-${TGT}/v0-train_mined_t1.09_filter${L}_u2u_augp${AUGP}augM${AUGM}/checkpoint_best.pt
RESULTS_PATH=/data/sls/scratch/clai24/lexicon/exp/textless_s2ut_gen/${SRC}-${TGT}_v0-train_mined_t1.09_filter${L}_u2u_augp${AUGP}augM${AUGM}_beam${BEAM}/
#######################################################################
...
for ORIG_GEN_SUBSET in test_epst_filter${L} test_epst test_fleurs; do
...
if [ $stage -eq 0 ]; then 
    ...
    fairseq-generate $DATA_ROOT \
      ...
      --task unit_to_unit_aug --target-is-code --target-code-size 1000 --vocoder code_hifigan \
      --source-is-code --source-code-size 1000 \
      --is-unit-aug --aug-p ${AUGP} --aug-M ${AUGM} \
