...
class WordSegF1(): 
    ...
    def __init__(self, args):
        ...
        self.get_pickles()
        self.phn_compositio_dict = defaultdict(int)
    ...
    def get_pickles(self): 
        ...
        for uttid in self.uttids:
            if self.args.parse_alg == "top_down":
                span_pkl = f'{self.args.parse_out_dir}/{self.args.model_version}.{self.args.split}.{self.args.parse_alg}/{uttid}-minsplitpmi{self.args.min_pmi}.pkl'
            elif self.args.parse_alg == "bottom_up":
                span_pkl = f'{self.args.parse_out_dir}/{self.args.model_version}.{self.args.split}.{self.args.parse_alg}/{uttid}-minmergepmi{self.args.min_pmi}.pkl'
    ...
    def cal_corpus_f1(self):
        ...
        for utt in self.uttids:
            ...
            # construct subword lexicons on best_split 
            if span_levels != {} and best_split != None:
                self.add_to_subword_lex(span_levels[best_split], utt)
        ...
        # lexical entry > X is considered
        lexical_entry_min_cnt = 5
        self.phn_compositio_dict = {k: v for k, v in self.phn_compositio_dict.items() if v >= lexical_entry_min_cnt}
        self.phn_compositio_dict = dict(sorted(self.phn_compositio_dict.items(), key=lambda x: x[1], reverse=True))
        print(f"sub-word lexicon is {self.phn_compositio_dict}")
        if self.args.parse_alg == "bottom_up":
            pth = os.path.join(self.args.parse_out_dir, f"{self.args.model_version}.{self.args.split}.{self.args.parse_alg}", f"subword_lexicon{lexical_entry_min_cnt}-minmergepmi{self.args.min_pmi}.pkl")
        elif self.args.parse_alg == "top_down": 
            pth = os.path.join(self.args.parse_out_dir, f"{self.args.model_version}.{self.args.split}.{self.args.parse_alg}", f"subword_lexicon{lexical_entry_min_cnt}-minsplitpmi{self.args.min_pmi}.pkl")
        with open(pth, "wb") as file:
            pickle.dump(self.phn_compositio_dict, file)
    ...
    def process_topdown_spans(self, my_dict, uttid): 
        ...
        if args.top_down_recursive_search: 
        ...
        return greedy_extended_span_levels
    ...
    def add_to_subword_lex(self, parses, uttid):
        oracle_phone_seg = self.utt2phnseg[uttid]
        ...
        for seg in parses: 
            ...
            if len(phns) > 1: 
                self.phn_compositio_dict[tuple(phns)] += 1
...
if __name__ == '__main__':
    ...
    parser.add_argument("--parse-alg", type=str, 
                        ...
                        default="bottom_up")
    parser.add_argument("--top-down-recursive-search", action="store_true", 
                        help="for top-down parse, recursively find segmentation with best F1s.")
